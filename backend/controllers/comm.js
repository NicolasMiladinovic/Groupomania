...
exports.getallcomms = (req, res, next) => {
    const post_id = req.params.id;
    db.query(`SELECT Users.pseudo, Users.name, Users.firstname, Comms.id, Comms.content, Comms.date, Comms.user_id FROM Users INNER JOIN Comms ON Users.id = Comms.user_id WHERE Comms.post_id = ? ORDER BY id desc`, [post_id], function (err, result) {
        if (err) {
            console.log(err);
            return res.status(400).json("error");
        } else {
